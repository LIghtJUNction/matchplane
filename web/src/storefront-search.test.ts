...
describe("public storefront search", () => {
  ...
  it("returns only complete canonical products and strips private fields", async () => {
    query.mockResolvedValue({ rows: [
      {
        ...
        attributes: {
          ...
          attachments: [{
            kind: "image",
            attachment_ref: "media://hosted/50000000-0000-4000-8000-000000000001",
            file_name: "camera.webp",
            media_type: "image/webp",
            metadata: { public_url: "https://tracking.example/camera.webp", private_key: "secret" },
          }],
        },
        terms: {
    ...
    const products = await searchPublicStoreOffers({ stores: [store], narrative: "旅行相机" });
    ...
    expect(products).toEqual([expect.objectContaining({
      ...
      attributes: {
        description: "适合旅行拍摄",
        brand: "Example",
        attachments: [{ kind: "image", file_name: "camera.webp", media_type: "image/webp", public_url: "/api/store-media/50000000-0000-4000-8000-000000000001" }],
      },
      terms: { pricing_mode: "fixed", amount_minor: "1299900", currency: "CNY", currency_scale: 2 },
    })]);
  ...
  it("rejects unsafe image URLs instead of presenting a fabricated product card", async () => {
    query.mockResolvedValue({ rows: [{
      ...
      attributes: {
        ...
        attachments: [{ kind: "image", metadata: { public_url: "https://user:secret@example.test/a.png" } }],
      },
    ...
    await expect(searchPublicStoreOffers({ stores: [store], narrative: "相机" })).resolves.toEqual([]);
  });
});
