...
export async function forwardPaymentAdmin(
  ...
  if (method === "GET") {
    ...
    try {
      ...
      return paymentAdminErrorResponse(error) ?? NextResponse.json({ error: "tenant scope invalid" }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "支付管理请求 JSON 无效" }, { status: 400 });
    }
    let tenantId: string;
    try {
      tenantId = pinnedTenantId(typeof input.tenant_id === "string" ? input.tenant_id : null);
    } catch (error) {
      return paymentAdminErrorResponse(error) ?? NextResponse.json({ error: "tenant scope invalid" }, { status: 400 });
    }
  ...
  try {
    response = await fetch(upstream, { method, headers, body, cache: "no-store" });
  } catch (error) {
...
export function paymentAdminErrorResponse(error: unknown): Response | null {
  if (error instanceof TenantScopeError) {
    return NextResponse.json({ error: "支付管理只能访问当前根平台 tenant" }, { status: 403 });
  }
  if (error instanceof Error && error.message === "MATCHPLANE_ROOT_TENANT_ID is not configured") {
