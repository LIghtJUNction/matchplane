...
export function verifyFederationEnrollment(
...
): { ok: true; value: VerifiedFederationEnrollment } | { ok: false; error: string } {
  if (!isRecord(value)) return { ok: false, error: "入驻报文必须是 JSON 对象" };
  if (value.protocol !== FEDERATION_PROTOCOL) return { ok: false, error: "不支持的联邦协议版本" };
  ...
  if (!nodeId || !isUuid(nodeId)) return { ok: false, error: "nodeId 必须是 UUID" };
  if (!slug || !SLUG_PATTERN.test(slug)) return { ok: false, error: "slug 格式无效" };
  if (!displayName) return { ok: false, error: "displayName 必须填写" };
  if (!endpoint) return { ok: false, error: "endpoint 必须是安全的 HTTPS MCP 地址" };
  if (!mcpServerKey || !SERVER_KEY_PATTERN.test(mcpServerKey)) {
    return { ok: false, error: "mcpServerKey 格式无效" };
  }
  if (!publicKey || !signature || !isBase64(publicKey) || !isBase64(signature)) {
  ...
  if (!normalizedEndpoint) return { ok: false, error: "endpoint 必须是安全的 HTTPS MCP 地址" };
  const manifest = value.manifest;
  if (!isRecord(manifest)) return { ok: false, error: "manifest 必须是 JSON 对象" };
  ...
  try {
    ...
    if (keyBytes.length < 32 || keyBytes.length > 4_096 || signatureBytes.length !== 64) {
      return { ok: false, error: "联邦公钥或签名长度无效" };
    }
    const key = createPublicKey({ key: keyBytes, format: "der", type: "spki" });
    signatureValid = verify(null, Buffer.from(signedPayload), key, signatureBytes);
  } catch {
...
export function validateFederationTokenEnv(value: unknown): string | null {
  ...
  return typeof value === "string" && ENV_NAME_PATTERN.test(value) ? value : null;
}
...
function validateManifest(manifest: Record<string, unknown>, slug: string): string | null {
  const serialized = canonicalJson(manifest);
  if (serialized.length > 64 * 1024) return "manifest 不能超过 64 KiB";
  if (manifest.apiVersion !== "matchplane.subplatform/v1" || manifest.rootApiVersion !== "v1") {
    return "manifest API 版本不受支持";
  }
  const id = stringValue(manifest.id, 128);
  const manifestSlug = stringValue(manifest.slug, 63);
  if (!id || !/^[a-z0-9][a-z0-9._-]{1,127}$/.test(id)) return "manifest.id 格式无效";
  if (!manifestSlug || manifestSlug !== slug || !SLUG_PATTERN.test(manifestSlug)) {
    return "manifest.slug 与入驻 slug 不一致";
  }
  const requiredScopes = manifest.requiredScopes;
  if (requiredScopes !== undefined && (!Array.isArray(requiredScopes)
    || requiredScopes.length > 32
    || requiredScopes.some((item) => typeof item !== "string" || item.length < 1 || item.length > 128))) {
    return "manifest.requiredScopes 格式无效";
  }
  const agent = manifest.agent;
  if (agent !== undefined && !isRecord(agent)) return "manifest.agent 必须是对象";
  if (isRecord(agent)) {
    if (agent.protocol !== undefined && agent.protocol !== "matchplane.agent/v1") return "manifest.agent.protocol 不受支持";
    const tools = agent.mcpTools;
    if (tools !== undefined && (!Array.isArray(tools) || tools.length > 64
      || tools.some((item) => typeof item !== "string" || !SERVER_KEY_PATTERN.test(item)))) {
...
function normalizeFederationEndpoint(value: string, environment: string | undefined): string | null {
  try {
    const url = new URL(value);
    const isLoopback = url.hostname === "localhost" || url.hostname === "127.0.0.1" || url.hostname === "[::1]";
    if (url.protocol !== "https:" && !(environment !== "production" && url.protocol === "http:" && isLoopback)) return null;
...
function stringValue(value: unknown, maximum: number): string | null {
  return typeof value === "string" && value.trim().length >= 1 && value.length <= maximum ? value.trim() : null;
}
