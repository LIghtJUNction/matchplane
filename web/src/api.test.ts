...
import { clearPartySessionCache, readPartySession, savePartySession } from "./api";
...
describe("marketplace capability cache", () => {
  ...
  it("does not let an expired admin cache hide a valid dual-role capability", () => {
    ...
    savePartySession({ ...base, accessToken: "expired", accessTokenExpiresAt: new Date(Date.now() - 1_000).toISOString() }, "used-car", "admin");
    savePartySession({ ...base, accessToken: "active", accessTokenExpiresAt: new Date(Date.now() + 60_000).toISOString() }, "used-car", "both");
  ...
  it("does not reuse a capability after the Better Auth user changes", () => {
    savePartySession({
      tenantId: "123e4567-e89b-12d3-a456-426614174000",
      partyId: "223e4567-e89b-12d3-a456-426614174000",
      authUserId: "333e4567-e89b-12d3-a456-426614174000",
      role: "buyer",
      accessToken: "alice",
      accessTokenExpiresAt: new Date(Date.now() + 60_000).toISOString(),
    }, "used-car", "buyer");

    expect(readPartySession("buyer", "used-car", undefined, "444e4567-e89b-12d3-a456-426614174000")).toBeNull();
  });
