  ...
  const launcher = portalNode
    ? createPortal(
        <Button
          ...
          size="sm"
          type="button"
          aria-label={isZh ? "问选货员" : "Ask assistant"}
          aria-haspopup="dialog"
          aria-expanded={open}
          onClick={() => onOpenChange(true)}
        >
          <MessageSquareMore size={17} aria-hidden="true" />
          <span>{isZh ? "问选货员" : "Ask assistant"}</span>
        </Button>,
        portalNode,
      )
    : null;
      ...
      {isDesktop ? (
        <Dialog open={open} onOpenChange={onOpenChange}>
          <DialogContent
            ...
            closeButton={false}
            closeLabel={isZh ? "关闭选货员" : "Close shopping assistant"}
            frame={false}
            viewportProps={{ className: "desktop-clerk-dialog-viewport" }}
          >
              ...
              <DialogClose
                render={
                  <Button
                    ...
                    type="button"
                    aria-label={
                      isZh ? "关闭选货员" : "Close shopping assistant"
                    }
                  >
                    <X aria-hidden="true" />
                  </Button>
