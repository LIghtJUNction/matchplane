...
export function PlatformAiConfigPanel({ rootRole, onNotice }: { rootRole?: string | null; onNotice: (message: string) => void }) {
  ...
  function apply(current: ManagedPlatformRouterConfig) {
    setConfig(current); setEndpoint(current.endpoint); setModel(current.model); setProtocol(current.protocol); setEnabled(current.enabled);
  }
  ...
  return (
    <section className="surface root-email-config platform-ai-config" aria-labelledby="platform-ai-config-title">
      ...
      <div className="seller-upload-form">
        <label htmlFor="platform-ai-endpoint"><span>模型网关地址</span><Input id="platform-ai-endpoint" value={endpoint} disabled={!canEdit || loading} onChange={(event) => setEndpoint(event.target.value)} placeholder="https://api.example.com/v1/chat/completions" inputMode="url" /></label>
        <label htmlFor="platform-ai-model"><span>模型</span><Input id="platform-ai-model" value={model} disabled={!canEdit || loading} onChange={(event) => setModel(event.target.value)} placeholder="provider/model" /></label>
