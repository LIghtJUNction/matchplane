      ...
      <div className="remote-store-list" aria-label="已接入的远程店铺">
        {bindings.length ? bindings.map((binding) => (
          <div className="remote-store-row" key={binding.id}>
            ...
            <span className="remote-store-copy"><strong>{binding.displayName}</strong><small>{binding.endpoint}</small><small>{bindingStatusLabel(binding)}{binding.lastHealthAt ? " · 最近检查 " + new Date(binding.lastHealthAt).toLocaleString() : ""}</small></span>
