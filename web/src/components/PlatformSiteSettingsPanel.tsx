...
import { Globe2, Search, Save, ShieldCheck } from "lucide-react";
...
export function PlatformSiteSettingsPanel({
  ...
  const lookup = async () => {
    ...
    try {
      ...
      onNotice(error instanceof Error ? error.message : "自动查询失败，请手动填写");
    } finally {
      setLookingUp(false);
    }
  };
  ...
  return (
    <section className="surface site-settings-panel" aria-labelledby="site-settings-title">
      ...
      <form className="site-settings-form" onSubmit={save}>
        ...
        <label htmlFor="site-icp-url"><span>备案查询链接（可选）</span><input id="site-icp-url" type="url" value={icpRecordUrl} onChange={(event) => setIcpRecordUrl(event.target.value)} placeholder="https://…" inputMode="url" /></label>
        <label htmlFor="site-psb-number"><span>公安备案号（可选）</span><input id="site-psb-number" value={publicSecurityNumber} onChange={(event) => setPublicSecurityNumber(event.target.value)} placeholder="例如：京公网安备00000000000000号" maxLength={128} /></label>
        <label htmlFor="site-psb-url"><span>公安备案链接（可选）</span><input id="site-psb-url" type="url" value={publicSecurityUrl} onChange={(event) => setPublicSecurityUrl(event.target.value)} placeholder="https://…" inputMode="url" /></label>
        <div className="site-settings-actions">
          <p><ShieldCheck size={16} aria-hidden="true" />{settings?.configured ? "当前已发布备案资料" : "尚未发布备案资料"}{settings?.lookup_checked_at ? ` · 最近查询 ${formatDate(settings.lookup_checked_at)}` : ""}</p>
          <button className="button button-dark" type="submit" disabled={saving || loading}><Save size={16} aria-hidden="true" />{saving ? "保存中…" : "保存备案信息"}</button>
        </div>
      </form>
    </section>
  );
