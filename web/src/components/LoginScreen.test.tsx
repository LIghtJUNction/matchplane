...
const api = vi.hoisted(() => ({
  ...
  getMallLegalDocuments: vi.fn(async () => ({
    mallName: "MatchPlane",
    documents: {
      terms: { content: "协议", version: 7, updatedAt: "2026-08-21T00:00:00.000Z" },
      privacy: { content: "隐私", version: 9, updatedAt: "2026-08-21T00:00:00.000Z" },
    },
  })),
...
vi.mock("../subplatform", () => ({
  resolveSubplatform: () => ({ slug: "root", path: "/", brandName: "MatchPlane", label: "", description: "" }),
  loadSubplatform: async () => ({ slug: "root", path: "/", brandName: "MatchPlane", label: "", description: "" }),
}));

import { LoginScreen } from "./LoginScreen";

beforeEach(() => {
  vi.stubGlobal("fetch", vi.fn(async () => new Response(JSON.stringify({ passkey: true }), { status: 200, headers: { "content-type": "application/json" } })));
...
describe("LoginScreen", () => {
  it("requires an explicit agreement before a new account can be submitted", async () => {
    ...
    const consent = await screen.findByRole("checkbox", { name: /用户协议.*隐私政策/ });
