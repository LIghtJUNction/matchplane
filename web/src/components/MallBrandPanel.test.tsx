...
const api = vi.hoisted(() => ({
  ...
  getMallSettings: vi.fn(async () => ({
    ...
    activeProductTitleCount: 1,
    placeholderPhrases: [],
  })),
  saveMallLegalDocuments: vi.fn(),
  saveMallSettings: vi.fn(
    async ({
      name,
      expectedVersion,
    }: {
...
describe("MallBrandPanel", () => {
  it("saves the configured public mall name", async () => {
    ...
    expect(api.saveMallSettings).toHaveBeenCalledWith({
      name: "新商城",
      expectedVersion: 3,
    });
  });

  it("adds every active product title to the home typewriter settings", async () => {
    const user = userEvent.setup();
    render(<MallBrandPanel rootRole="rootSuperAdmin" onNotice={vi.fn()} />);

    await user.click(
      await screen.findByRole("button", { name: "加入全部商品标题" }),
    );
