...
export function MallCatalogModeration({
  onNotice,
}: {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      setOffers(
        await getMarketplaceOfferAdminRecords({ status: "draft", limit: 50 }),
      );
    } catch (error) {
  ...
  const approve = async (offer: MarketplaceOfferAdminRecord) => {
    ...
    setFailure(null);
    try {
      const activated = await activateMarketplaceOffer({
        offerId: offer.offer_id,
        tenantId: offer.tenant_id,
        expectedVersion: offer.version,
      });
      setOffers((current) =>
        current.filter((candidate) => candidate.offer_id !== offer.offer_id),
      );
      onNotice(
        activated.catalog_sync?.synced === false
          ? `商品已发布；外部店铺目录稍后重试同步：${activated.catalog_sync.error ?? "暂时不可用"}`
          : "商品已通过审核并进入公开目录",
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : "商品审核失败";
      setFailure({ offerId: offer.offer_id, message });
      onNotice(message);
    } finally {
      setApproving(null);
    }
  ...
  return (
    <section
      className="mall-catalog-moderation"
      aria-labelledby="mall-catalog-moderation-title"
    >
      ...
      {offers.length ? (
        <ul className="mall-catalog-moderation-list">
          {offers.map((offer) => (
            <li key={offer.offer_id}>
              ...
              <div
                className="mall-catalog-product-action"
                aria-busy={approving === offer.offer_id}
              >
                ...
                <button
                  ...
                  title={
                    isReadyForPublication(offer)
                      ? undefined
                      : "补齐店铺、图片、描述和有效价格后才能发布"
                  }
                ...
                {failure?.offerId === offer.offer_id ? (
                  <p className="mall-catalog-product-error" role="alert">
                    {failure.message}
                  </p>
                ) : null}
              </div>
        ...
        <p className="mall-catalog-moderation-empty">
          {loading ? "正在读取待审核商品…" : "目前没有待审核商品。"}
        </p>
      )}
    </section>
  );
