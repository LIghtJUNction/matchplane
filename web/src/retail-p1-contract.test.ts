...
describe("public marketplace accessibility and layout contract", () => {
  ...
  it("keeps actual muted text colors above the normal-text contrast floor", () => {
    ...
    for (const theme of [
      rule(retailCss, ":root"),
      rule(retailCss, ':root[data-theme="dark"]'),
    ]) {
      for (const surface of [
        ...
        expect(
          contrast(color(theme, "--retail-muted"), color(theme, surface)),
  ...
  it("retains explicit focus rings and coarse-pointer target sizing", () => {
    ...
    expect(rule(rootMarketplaceCss, ".match-chat-starter-card")).toContain(
      "min-height: 2.75rem;",
    );
  });
  ...
  it("keeps a compact entry and a responsive product-first catalog", () => {
    ...
    expect(rule(rootMarketplaceCss, ".root-marketplace-entry")).toContain(
      "min-height: 0;",
    );
    expect(rule(rootMarketplaceCss, ".root-marketplace-entry-frame")).toContain(
      "grid-template-columns: minmax(0, 0.9fr) minmax(0, 1.4fr);",
    );
    expect(rule(rootMarketplaceCss, ".home-chat")).toContain(
      "box-shadow: none;",
    );
    expect(rule(rootMarketplaceCss, ".home-chat-form")).toContain(
      "background: var(--retail-surface);",
    );
    expect(rule(rootMarketplaceCss, ".match-chat-starter-grid")).toContain(
      "flex-wrap: wrap;",
    );
    expect(
      rule(rootMarketplaceCss, ".root-marketplace-products-grid"),
    ).toContain("repeat(4, minmax(0, 1fr))");
    const mobile = rootMarketplaceCss.slice(
      rootMarketplaceCss.indexOf("@media (max-width: 760px)"),
    );
    expect(rule(mobile, ".root-marketplace-entry-frame")).toContain(
      "grid-template-columns: minmax(0, 1fr);",
    );
    expect(rule(mobile, ".root-marketplace-products-grid")).toContain(
      "repeat(2, minmax(0, 1fr))",
    );
    expect(rule(mobile, ".root-marketplace-inline-categories")).toContain(
      "overflow-x: auto;",
    );
    expect(rootMarketplaceCss).toContain(
      "@media (prefers-reduced-motion: reduce)",
    );
  });

  it("retains readable metadata and expands active conversations on one axis", () => {
    expect(rule(rootMarketplaceCss, ".root-marketplace-privacy")).toContain(
      "color: var(--retail-muted);",
    );
    expect(rule(rootMarketplaceCss, ".root-marketplace-scroll-cue")).toContain(
      "min-height: 2.75rem;",
    );
    expect(
      rule(rootMarketplaceCss, ".root-marketplace-products-heading span"),
    ).toContain("color: var(--retail-muted);");
    expect(
      rule(
        rootMarketplaceCss,
        ".root-marketplace-entry:has(.home-chat.has-conversation) .root-marketplace-entry-frame",
      ),
    ).toContain("grid-template-columns: minmax(0, 1fr);");
    expect(
      rule(
        rootMarketplaceCss,
        ".root-marketplace-entry:has(.home-chat.has-conversation) .root-marketplace-scroll-cue",
      ),
