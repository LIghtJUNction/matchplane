...
import {
  getManagedNationalIdentityConfig,
  saveManagedNationalIdentityConfig,
  type NationalIdentityEndpointMode,
} from "../../../../../src/lib/national-identity-config";
...
export async function GET(request: Request): Promise<Response> {
  ...
  return NextResponse.json({ config: getManagedNationalIdentityConfig() }, { headers: { "cache-control": "no-store" } });
}
...
export async function PATCH(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!value || typeof value !== "object" || Array.isArray(value)) return error("国家网络身份认证配置必须是对象", 400);
  ...
  try {
    const config = saveManagedNationalIdentityConfig({
      enabled: body.enabled === true,
      clientId: text(body.clientId),
      clientSecret: optionalText(body.clientSecret),
      endpointMode: body.endpointMode === "endpoints" ? "endpoints" : "discovery" as NationalIdentityEndpointMode,
      discoveryUrl: optionalText(body.discoveryUrl),
    ...
    return error(cause instanceof Error ? cause.message : "国家网络身份认证配置保存失败", 400);
  }
}
...
async function requireMallOwner(request: Request, write: boolean): Promise<true | Response> {
  ...
  if (write ? role !== "rootSuperAdmin" : role !== "rootSuperAdmin" && role !== "rootAdmin") {
    return error(write ? "只有商城负责人可以保存国家网络身份认证配置" : "只有商城后台人员可以查看国家网络身份认证配置", 403);
  }
  return true;
}
