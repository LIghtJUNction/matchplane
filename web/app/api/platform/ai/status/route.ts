  ...
  return NextResponse.json(
    {
      router: {
        ...
        maxSteps: boundedInteger(process.env.MATCHPLANE_ROUTER_AI_MAX_STEPS, 8, 1, 16),
        maxFanout: boundedInteger(process.env.MATCHPLANE_ROUTER_AI_MAX_FANOUT, 4, 1, 16),
        requestsPerHour: boundedInteger(process.env.MATCHPLANE_ROUTER_AI_REQUESTS_PER_HOUR, 120, 1, 10_000),
        globalRequestsPerHour: boundedInteger(process.env.MATCHPLANE_ROUTER_AI_GLOBAL_REQUESTS_PER_HOUR, 120, 1, 100_000),
      },
      auth: {
...
function parseToolMode(value: string | undefined): "auto" | "required" | "disabled" {
  ...
  return normalized === "required" || normalized === "disabled" ? normalized : "auto";
}
