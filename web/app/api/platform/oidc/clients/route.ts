...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!registrationId || !isUuid(registrationId)) {
    return NextResponse.json(
      { error: "subplatformRegistrationId 必须是 UUID" },
      { status: 400 },
    );
  }
  ...
  if (!clientName || clientName.length > 120) {
    return NextResponse.json(
      { error: "clientName 必须为 1..120 个字符" },
      { status: 400 },
    );
  }
  const redirectUris = validateUriList(input.redirectUris, "redirectUris", 10);
  if (!redirectUris.ok)
    return NextResponse.json({ error: redirectUris.error }, { status: 400 });
  const clientUri = validateOptionalUri(input.clientUri, "clientUri");
  if (!clientUri.ok)
    return NextResponse.json({ error: clientUri.error }, { status: 400 });
  ...
  if (postLogoutRedirectUris && !postLogoutRedirectUris.ok) {
    return NextResponse.json(
      { error: postLogoutRedirectUris.error },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      {
        ...
        secretHandling:
          "client_secret 只在本次响应返回；必须保存于子平台服务端密钥存储。",
      },
      {
        status: 201,
    ...
    return NextResponse.json(
      { error: "OIDC 客户端创建失败，请确认 Better Auth 迁移已完成" },
      { status: 409 },
    );
  }
}
  ...
  try {
    if (action === "rotate-secret") {
      ...
      return NextResponse.json(
        {
          ...rotated,
          secretHandling: "新 client_secret 只在本次响应返回；旧密钥立即失效。",
        },
        { headers: { "cache-control": "no-store", pragma: "no-cache" } },
      );
    }
    ...
    if (action === "enable" || action === "disable") {
      ...
      return NextResponse.json(
        { clientId, disabled: action === "disable" },
        { headers: { "cache-control": "no-store" } },
      );
    }
    ...
    if (action !== "update") {
      return NextResponse.json(
        { error: "action 必须是 update、rotate-secret、enable 或 disable" },
        { status: 400 },
      );
    }
    const update: {
    ...
    if (input.redirectUris !== undefined) {
      const redirectUris = validateUriList(
        input.redirectUris,
        "redirectUris",
        10,
      );
...
export async function DELETE(request: Request): Promise<Response> {
  const guard = await requireRootAdmin(request);
  if (guard.response) return guard.response;
  const clientId = requestSearchParams(request).get("client_id")?.trim();
  if (!clientId || clientId.length > 256)
    return NextResponse.json({ error: "client_id 无效" }, { status: 400 });
  try {
    const result = await authDatabase.query(
      `UPDATE "oauthClient"
    ...
    if (result.rowCount !== 1)
      return NextResponse.json({ error: "OIDC 客户端不存在" }, { status: 404 });
    await authDatabase.query(
      `UPDATE "oauthRefreshToken"
          SET "revoked" = COALESCE("revoked", clock_timestamp())
        WHERE "clientId" = $1`,
...
function validateUriList(
  ...
  if (new Set(values).size !== values.length)
    return { ok: false, error: `${field} 不得包含重复地址` };
...
function validateUri(
  ...
  try {
    ...
    if (uri.protocol !== "https:" && !(uri.protocol === "http:" && localhost)) {
      return {
        ok: false,
        error: `${field} 必须使用 HTTPS（本地开发仅允许 localhost）`,
      };
    }
