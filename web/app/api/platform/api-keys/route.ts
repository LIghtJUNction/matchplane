...
export async function POST(request: Request): Promise<Response> {
  ...
  if (
    ...
    input.name.trim().length > 32
  ) {
    return NextResponse.json(
      { error: "name must contain 1..=32 characters" },
      { status: 400 },
    );
  }
  if (
    ...
    (!Number.isInteger(input.expiresIn) ||
      input.expiresIn < 24 * 60 * 60 ||
      input.expiresIn > MAX_KEY_TTL_SECONDS)
  ) {
    return NextResponse.json(
      { error: "expiresIn must be between one day and one year" },
      { status: 400 },
    );
  }
  ...
  if (!(await belongsToConfiguredTenant(input.organizationId))) {
    return NextResponse.json(
      { error: "API Key 只能签发给当前 root tenant 下的平台组织" },
      { status: 403 },
    );
  }
  ...
  if (permissions === null) {
    return NextResponse.json(
      { error: "permissions contains an unsupported action" },
      { status: 400 },
    );
  }
  if (input.agentSide !== undefined && !isAgentSide(input.agentSide)) {
    return NextResponse.json(
      { error: "agentSide must be demand, supply, or both" },
      { status: 400 },
    );
  }
  if (input.agentRole !== undefined && !isAgentRole(input.agentRole)) {
    return NextResponse.json(
      { error: "agentRole is a deprecated compatibility alias" },
      { status: 400 },
    );
  }
  if (input.agentSide !== undefined && input.agentRole !== undefined) {
    ...
    if (roleSide !== input.agentSide) {
      return NextResponse.json(
        { error: "agentSide 与兼容字段 agentRole 不一致" },
        { status: 400 },
      );
    }
  }
  if (
    (input.agentSide !== undefined || input.agentRole !== undefined) &&
    !permissions?.marketplace?.includes("write")
  ) {
    return NextResponse.json(
      { error: "agentSide requires marketplace:write permission" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "API Key 创建失败；请确认你是该平台的成员管理员" },
      { status: 403 },
    );
  }
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  const organizationId = requestSearchParams(request).get("organizationId");
  ...
  if (!(await belongsToConfiguredTenant(organizationId))) {
    return NextResponse.json(
      { error: "API Key 只能读取当前 root tenant 下的平台组织" },
      { status: 403 },
    );
  }
...
export async function PATCH(request: Request): Promise<Response> {
  ...
  if (!input.keyId || !isUuid(input.keyId))
    return NextResponse.json(
      { error: "keyId must be a UUID" },
      { status: 400 },
    );
  ...
  if (input.enabled === undefined && input.name === undefined)
    return NextResponse.json(
      { error: "enabled or name is required" },
      { status: 400 },
    );
  if (
...
async function requireKeyManager(
  request: Request,
  organizationId: string,
  ...
  if (!(await belongsToConfiguredTenant(organizationId))) {
    return {
      response: NextResponse.json(
        { error: "API Key 只能管理当前 root tenant 下的平台组织" },
        { status: 403 },
      ),
    };
  }
...
async function keyBelongsToOrganization(
  keyId: string,
  organizationId: string,
): Promise<boolean> {
 ...
 */
async function belongsToConfiguredTenant(
  organizationId: string,
): Promise<boolean> {
...
function normalizePermissions(
  ...
  for (const [resource, actions] of Object.entries(permissions)) {
    ...
    if (!allowed || !Array.isArray(actions) || actions.length === 0)
      return null;
...
function isAgentRole(
  value: unknown,
): value is NonNullable<ApiKeyRequest["agentRole"]> {
  return value === "buyer" || value === "seller" || value === "both";
}

function isAgentSide(
  value: unknown,
): value is NonNullable<ApiKeyRequest["agentSide"]> {
  return value === "demand" || value === "supply" || value === "both";
}

function roleToSide(
  value: ApiKeyRequest["agentRole"],
): ApiKeyRequest["agentSide"] | undefined {
  return value === "buyer"
    ? "demand"
    : value === "seller"
      ? "supply"
