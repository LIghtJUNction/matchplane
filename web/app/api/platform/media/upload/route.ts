...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!(await isMountedPlatformPath(input.scope.platform_path)))
    return jsonError("当前平台路径尚未激活", 404);

  const hostedStore = await readHostedStore(input.scope);
  if (hostedStore) {
    if (input.attachment.kind !== "image")
      return jsonError("托管店铺当前只接受商品图片", 400);
    if (input.attachment.size_bytes > DEFAULT_MAX_MEDIA_BYTES)
      return jsonError("单张商品图片不能超过 25 MiB", 413);
    if (actor.access === "session") {
      const member = await authDatabase.query(
        `SELECT 1 FROM "member"
          ...
          LIMIT 1`,
        [
          hostedStore.organizationId,
          actor.subject,
          ["owner", "admin", "subplatform_admin"],
        ],
      );
      if (!actor.isRootAdministrator && member.rowCount !== 1)
        return jsonError("只有店主或店铺运营可以上传商品图片", 403);
    ...
    try {
      ...
      if (error instanceof HostedMediaQuotaExceeded)
        return jsonError(error.message, 413);
      if (error instanceof HostedMediaInvalidImage)
  ...
  if (actor.access === "session") {
    ...
    if (
      ...
      (!scope ||
        scope.tenantId !== input.scope.tenant_id ||
        scope.domainId !== input.scope.domain_id)
    ) {
  ...
  if (!execution.ok)
    return jsonError(
      readError(execution.payload) ?? "子平台媒体适配器暂时不可用",
      execution.status >= 400 ? execution.status : 502,
    );

  const extracted = extractMcpMediaUploadResult(execution.payload);
  if (!extracted.ok) return jsonError(extracted.error, 502);
  const response = parseMediaUploadResponse(
    extracted.value,
    input.request_id,
    maximumBytes,
  );
...
async function readHostedStore(scope: {
  tenant_id: string;
  domain_id: string;
  platform_path: string;
}): Promise<HostedStoreScope | null> {
...
async function persistHostedImage(input: {
  ...
  if (!storageRoot)
    throw new Error("MATCHPLANE_HOSTED_MEDIA_ROOT must be an absolute path");
  ...
  const canonicalStoreDirectory = await realpath(
    /* turbopackIgnore: true */ storeDirectory,
  );
  if (!canonicalStoreDirectory.startsWith(`${storageRoot}${path.sep}`))
    throw new Error("hosted media directory escaped its root");
  const filePath = path.join(
    /* turbopackIgnore: true */ canonicalStoreDirectory,
    storageKey,
  );
  ...
  try {
    ...
    if (
      BigInt(usage.rows[0]?.bytes ?? "0") + BigInt(bytes.byteLength) >
      1024n * 1024n * 1024n
    ) {
...
async function normalizeHostedImage(
  bytes: Buffer,
  originalName: string,
): Promise<{
  bytes: Buffer;
  ...
  try {
    ...
    if (
      !converted.data.byteLength ||
      converted.data.byteLength > DEFAULT_MAX_MEDIA_BYTES
    ) {
    ...
    throw new HostedMediaInvalidImage(
