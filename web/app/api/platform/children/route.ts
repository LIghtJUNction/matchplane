...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!(await isActivePlatformPathVisible(path, viewer))) {
    return NextResponse.json({ error: "platform is not available" }, { status: 404 });
  }
  ...
  const children = await readActiveDirectChildRoutes(path, rootTenantId, viewer);
  return NextResponse.json(
    {
      children: children.map(({ slug, path: childPath, displayName, description, capabilities, agentStages, agentSkills }) => ({
        slug,
        path: childPath,
        displayName,
        description,
