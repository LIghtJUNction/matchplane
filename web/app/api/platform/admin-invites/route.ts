...
export async function POST(request: Request): Promise<Response> {
  ...
  if ((session.user as { role?: string | null }).role !== "rootSuperAdmin")
    return error("只有超级管理员可以创建平台管理员邀请", 403);
  ...
  const expiresHours =
    typeof body.expiresHours === "number" && Number.isInteger(body.expiresHours)
      ? body.expiresHours
      : 24;
  if (!isEmail(email) || !(1 <= expiresHours && expiresHours <= 168))
    return error("邮箱或邀请有效期无效", 400);
