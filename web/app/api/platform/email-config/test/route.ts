...
export async function POST(request: Request): Promise<Response> {
  ...
  const user = session.user as { id: string; email?: string | null; emailVerified?: boolean; role?: string | null };
  if (user.role !== "rootSuperAdmin") return NextResponse.json({ error: "只有超级管理员可以测试根邮箱配置" }, { status: 403 });
  if (!user.email || user.emailVerified !== true) return NextResponse.json({ error: "请先验证超级管理员邮箱后再测试" }, { status: 409 });
  try {
