...
export async function PATCH(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (
      current.terms.version !== termsVersion ||
      current.privacy.version !== privacyVersion
    ) {
      await client.query("ROLLBACK");
      return jsonError("法律页面已被其他人更新，请刷新后重试", 409);
    }
    const terms = await updateDocument(
      client,
      tenantId,
      "terms",
      termsContent,
    ...
    const privacy = await updateDocument(
      client,
      tenantId,
      "privacy",
      privacyContent,
...
function mapDocuments(
  rows: LegalRow[],
): { terms: LegalDocument; privacy: LegalDocument } | null {
  ...
  return {
    ...
    privacy: {
      ...
      version: Number(privacy.version),
