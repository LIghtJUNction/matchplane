...
const RESERVED_SLUGS = new Set([
  "account", "admin", "api", "auth", "forgot-password", "login", "oauth", "register",
  "reset-password", "store-media", "stores",
]);

/** Public, contact-free flat store directory for the marketplace shell. */
export async function GET(request: Request): Promise<Response> {
  if (new URL(request.url).searchParams.get("mine") === "1") return readOwnedStores(request);
  if (new URL(request.url).searchParams.get("manage") === "1") return readManagedStores(request);
  ...
  if (!stores) return NextResponse.json({ error: "店铺目录暂时不可用" }, { status: 503 });
...
async function readManagedStores(request: Request): Promise<Response> {
  ...
  if (!session || (role !== "rootSuperAdmin" && role !== "rootAdmin")) return jsonError("当前账号没有商城运营权限", 403);
...
export async function POST(request: Request): Promise<Response> {
  ...
  const mallOperator = sessionUser.role === "rootSuperAdmin" || sessionUser.role === "rootAdmin";
  if (!mallOperator && sessionUser.emailVerified !== true && sessionUser.phoneNumberVerified !== true) {
    return jsonError("验证邮箱或手机号后才能开店", 403);
  }

  let input: { name?: unknown; slug?: unknown; description?: unknown };
  try {
  ...
  if (!slug || RESERVED_SLUGS.has(slug)) return jsonError("店铺地址只能使用 2 到 63 位小写字母、数字和短横线，且不能占用系统地址", 400);
  if (description === null) return jsonError("店铺简介不能超过 2000 个字符", 400);
  const tenantId = configuredTenantId();
  if (!tenantId) return jsonError("商城尚未完成初始化", 503);
  ...
  const eligibility = await authDatabase.query<{ owned: number; recent: number; slug_taken: boolean }>(
    `SELECT
       ...
       EXISTS(SELECT 1 FROM stores WHERE tenant_id = $1::uuid AND slug = $3) AS slug_taken`,
    [tenantId, session.user.id, slug],
  );
  if (Number(eligibility.rows[0]?.owned ?? 0) >= 5) return jsonError("每个账号最多创建 5 家托管店铺", 409);
  if (Number(eligibility.rows[0]?.recent ?? 0) >= 2) return jsonError("每小时最多创建 2 家店铺，请稍后再试", 429);
  if (eligibility.rows[0]?.slug_taken) return jsonError("这个店铺地址已经被使用", 409);
  ...
  try {
    organization = await auth.api.createOrganization({
      body: {
        ...
        metadata: { tenantId, integrationKind: "hosted", parentOrganizationId: rootOrganizationId },
      },
    }) as { id: string; name: string; slug: string };
  } catch (error) {
    console.error("hosted store organization creation failed", error);
    return jsonError("店铺地址已经被使用，请换一个地址", 409);
  }
  ...
  try {
    ...
    await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`matchplane:hosted-store:${session.user.id}`]);
    ...
    if (Number(count.rows[0]?.count ?? 0) >= 5) throw new StoreCreationConflict("每个账号最多创建 5 家托管店铺");
    ...
    if (Number(recent.rows[0]?.count ?? 0) >= 2) throw new StoreCreationConflict("每小时最多创建 2 家店铺，请稍后再试", 429);
    await client.query(
    ...
    const projected = await client.query(
      `UPDATE "organization"
        ...
        WHERE id = $1::uuid
        RETURNING id`,
      [organization.id, tenantId, domainId, rootOrganizationId, JSON.stringify({ storeId, integrationKind: "hosted" })],
    );
    if (projected.rowCount !== 1) throw new Error("hosted store organization projection failed");
    await client.query(
      `INSERT INTO stores
      ...
      [storeId, tenantId, organization.id, domainId, slug, name, description, session.user.id],
    );
    await client.query(
    ...
    await client.query(
      ...
      [randomUUID(), tenantId, domainId, `/${slug}`, session.user.id, JSON.stringify({ store_id: storeId, slug })],
    );
    await client.query("COMMIT");
  } catch (error) {
    await client?.query("ROLLBACK").catch(() => undefined);
    await authDatabase.query('DELETE FROM "organization" WHERE id = $1::uuid', [organization.id]).catch(() => undefined);
    if (error instanceof StoreCreationConflict) return jsonError(error.message, error.status);
    if (isUniqueViolation(error)) return jsonError("这个店铺地址已经被使用", 409);
  ...
  return NextResponse.json({
    store: { id: storeId, slug, path: `/${slug}`, displayName: name, description, integrationKind: "hosted", status: "active" },
