...
if [[ ${MATCHPLANE_BUILD_PACKAGES:-0} == 1 ]]; then
  ...
  packaging/scripts/archive.sh 0.1.7 target/release "$output_directory"
  tar --list --zstd --file "$output_directory/matchplane-0.1.7-linux-x86_64.tar.zst" >/dev/null
  if command -v dpkg-deb >/dev/null 2>&1; then
    packaging/ubuntu/build-deb.sh 0.1.7 target/release "$output_directory"
    dpkg-deb --info "$output_directory/matchplane_0.1.7_amd64.deb" >/dev/null
  fi
fi
