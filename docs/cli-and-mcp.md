...
命令自动读取主机的 root-owned 配置文件；无需手动加载环境变量。若需覆盖默认管理员，使用 `--email admin@matx.tech`；`auth passwd` 与旧的 `auth reset-password` 仍是兼容入口。非 TTY 自动化必须显式使用 `--password-stdin`，并让 secret manager 直接提供密码，不要把密码写进 shell 历史或命令参数。
...
`matchplane serve subplatform-builder` 只启动隔离构建器二进制；它不读取 `MATCHPLANE_DATABASE_URL`，也不提供 shell/MCP 工具。生产环境应通过独立容器或 systemd sandbox 给予它 `MATCHPLANE_SUBPLATFORM_BUILDER_WEB_URL`、callback token、上传只读目录、artifact 写目录和独立工作根，并显式配置 `bubblewrap`、来源 host allowlist 与构建时间上限。Bun 包可通过 `MATCHPLANE_SUBPLATFORM_BUILDER_BUN` 指定绝对路径（例如官方安装脚本生成的 `/opt/bun/bin/bun`）；未设置时使用服务 PATH 中的 `bun`，运行时版本由运营方选择，不写入子平台 manifest。
...
声明 `media.upload` 的包还可通过 `POST /api/platform/media/upload` 接收浏览器/Agent 的受限资料请求。根会先验证浏览器会话或具备 `media:upload` 权限的组织 API key，再读取 base64 envelope；浏览器会话还必须拥有当前子平台成员权限。根只转发临时 base64 envelope，子平台必须负责扫描、存储和图片/文档解析，并返回 `media://` 引用。默认请求大小为 25 MiB，可由 `MATCHPLANE_MEDIA_MAX_BYTES` 配置至 256 MiB 上限；反向代理必须同步配置，禁止用无界 body 解除所有限制。
