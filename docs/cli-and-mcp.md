...
声明 `media.upload` 的包还可通过 `POST /api/platform/media/upload` 接收浏览器/Agent 的受限资料请求。浏览器会话使用子平台成员权限；机器 Agent 使用组织 API key 的 `media:upload` 权限。根只转发临时 base64 envelope，子平台必须负责扫描、存储和图片/文档解析，并返回 `media://` 引用。默认请求大小为 100 MiB，可由 `MATCHPLANE_MEDIA_MAX_BYTES` 配置至 256 MiB 上限；反向代理必须同步配置，禁止用无界 body 解除所有限制。
