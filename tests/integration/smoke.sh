...
curl --fail-with-body --silent --request POST "$base_url/v1/orders" \
  --header 'content-type: application/json' --data-binary "@$seller_request" | jq -e '.duplicate == false' >/dev/null
wait_for 'seller order admission' "curl --fail --silent '$base_url/v1/orders/00000000-0000-7000-8000-000000008001' | jq -e '.status == \"open\"'"

curl --fail-with-body --silent --request POST "$base_url/v1/orders" \
  --header 'content-type: application/json' --data-binary "@$buyer_one_request" | jq -e '.duplicate == false' >/dev/null
curl --fail-with-body --silent --request POST "$base_url/v1/orders" \
  --header 'content-type: application/json' --data-binary "@$buyer_one_request" | jq -e '.duplicate == true' >/dev/null
wait_for 'first deterministic trade' "curl --fail --silent '$base_url/v1/markets/$market_id/trades' | jq -e 'length == 1 and .[0].price == \"100\" and .[0].quantity == \"3\"'"
wait_for 'first projected book' "curl --fail --silent '$base_url/v1/markets/$market_id/book' | jq -e '.sequence == 2 and (.asks | length) == 1 and .asks[0].price == \"100\" and .asks[0].quantity == \"2\"'"
...
curl --fail-with-body --silent --request POST "$base_url/v1/orders" \
  --header 'content-type: application/json' --data-binary "@$buyer_two_request" | jq -e '.duplicate == false' >/dev/null
wait_for 'post-restart snapshot recovery and trade' "curl --fail --silent '$base_url/v1/markets/$market_id/trades' | jq -e 'length == 2'"
wait_for 'empty projected book after second fill' "curl --fail --silent '$base_url/v1/markets/$market_id/book' | jq -e '.sequence == 3 and (.bids | length) == 0 and (.asks | length) == 0'"
